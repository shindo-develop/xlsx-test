...
import { htmlField, globalFunc, isExport } from './config.js';
...
// option
document.getElementById('floatingSearch').addEventListener('change', (event) => {
    const contentName = event.composedPath()[0].value
    const contentJson = excelJson[contentName];
    globalFunc.displayNone(htmlField.contentGroup);
    globalFunc.displayBlock(htmlField.contentTable);
